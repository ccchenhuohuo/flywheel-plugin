...
from acceptance_core import Run,create_run,read,write,digest,months,shift,readonly_sql,quoted
...
def rows_for(run,j):
    ...
    if f=='sample':return [{'path':'["a","b","c"]','product_id':'p1','title':'Example camera accessory','brand':'Example','raw_category':'Electronics','units':10,'amount':100}]
    result=[]
    for mm in ms:
        base={'month_dt':mm+'-01'}
...
def test_complete_execution_still_discloses_missing_inputs(completed):
    v=validate(completed,False)
    assert not v['errors'],v['errors']
    assert v['execution_state']=='limited' and v['business_verdict']=='undetermined'
...
def test_sealing_incomplete_run_never_means_pass(completed):
    ...
    v=validate(completed);render(completed);completed.seal(v)
    sealed=read(completed.path/'sealed.json')
    assert sealed['execution_state']=='incomplete' and sealed['business_verdict']=='undetermined'
    with pytest.raises(ValueError,match='封存'):completed.event('attempt')
