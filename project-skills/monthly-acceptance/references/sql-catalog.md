...
自动全景扫描由 scripts/acceptance_queries.py 固定规划并通过执行器取证。本目录 8 个 SQL 保留为人工算数示例，不能替代完整扫描计划。

这些是 Doris 只读示例，不是流水线。先固定国家、同一父类目、月份和粒度，再选择/改写查询；沿销量、金额两条线检查贡献，模型决定下钻深度。全部历史观察应记录实际覆盖与分页，查询失败不写成零。
...
- 提交语句以 SELECT/WITH/EXPLAIN 起始，不附末尾分号；当前 MCP 对首行注释可能返回空回包，对分号后自动追加 LIMIT 可能报错。空回包先核实实际执行状态。
