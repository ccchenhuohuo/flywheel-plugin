...
async def call(run, session, sql, label, *, parent=None, offset=None, page_size=None):
    ...
    for attempt in range(1,run.policy['retry_count']+2):
        ...
        attempt_ids.append(eid);t=time.monotonic()
        try:
            ...
            run.event('tool.finished',parent_event_id=eid,status='success',duration_seconds=round(time.monotonic()-t,3),
                evidence_id=evidence,rows=len(rows),actual_query=response.get('metadata',{}).get('query'))
            return evidence,response
        except asyncio.CancelledError:
            run.event('tool.finished',parent_event_id=eid,status='cancelled',duration_seconds=round(time.monotonic()-t,3))
            raise
        except Exception as exc:
            run.event('tool.finished',parent_event_id=eid,status='error',duration_seconds=round(time.monotonic()-t,3),
...
async def execute_job(run,session,j,force=False):
    ...
    if old and old['execution_status']=='success' and not force:
        run.job_rows(j['job_id'])
        return old
    run.writable();start=now();eids=[];offset=0;expected=None
    run.event('job.started',job_id=j['job_id'],family=j['family'],site=j['site'])
    write(run.path/'records'/('job-'+j['job_id']+'.json'),{'job_id':j['job_id'],'execution_status':'running','started_at':start})
    try:
        page_size=int(run.policy['page_size'])
        while True:
            eid,data=await call(run,session,paginated(j,page_size,offset),j['family'],parent=j['job_id'],offset=offset,page_size=page_size)
        ...
        record={**{k:v for k,v in j.items() if k!='sql'},'sql_hash':digest(j['sql']),'execution_status':'success',
            'evidence_ids':eids,'expected_rows':expected,'row_count':offset,'started_at':start,'finished_at':now()}
        write(run.path/'records'/('job-'+j['job_id']+'.json'),record)
        run.job_rows(j['job_id'])
        run.event('job.finished',job_id=j['job_id'],status='success',rows=offset,pages=len(eids))
        ...
        write(run.path/'records'/('job-'+j['job_id']+'.json'),{'job_id':j['job_id'],'execution_status':'cancelled',
            'evidence_ids':eids,'started_at':start,'finished_at':now(),'reason':'execution interrupted'})
        run.event('job.finished',job_id=j['job_id'],status='cancelled')
        raise
    except Exception as exc:
        write(run.path/'records'/('job-'+j['job_id']+'.json'),{'job_id':j['job_id'],'execution_status':'failed',
            'evidence_ids':eids,'error':str(exc),'started_at':start,'finished_at':now()})
        run.event('job.finished',job_id=j['job_id'],status='failed',error=str(exc))
...
async def scan(run,jobs,config=None,force=False):
    async with connect(run,config) as session:
        for j in jobs:await execute_job(run,session,j,force=force)
