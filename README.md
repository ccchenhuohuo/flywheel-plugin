...
Claude Code 插件：用受治理的只读 MCP 做大盘市场分析——趋势、份额、增长与贡献、TopN、价格带、
品牌集中度、标准类目与平台原始类目导航、单 SKU 搜索与逐月轨迹。
...
本仓库只包含分发件：插件清单、MCP 端点配置与方法论 Skill。

支撑它的两层——Doris 治理表构建管线（月度全量重算、append-only 快照发布、十类零行断言门禁）
与薄 MCP 服务端（12 个只读工具，服务端算全部可见数字）——在内部私有仓库，不随插件分发。
